...
2.  Suppose you use a binary search tree to store the words from the 
    ...
    > the worst case would be the length of the dictionary
...
3.  Suppose you use a prefix tree to store the words from the `dictionary.
    ...
    > the worst case would be the length of the longest word in the dictionary

4.  Why is the prefix tree better than a binary search tree for implementing 
    Boggle?

    > it takes less comparisons to search for words, and it generally uses less
	  memory for large input size

5.  Suppose you do not use `IsPrefix` in your `SolveBoard` implementation.  
    How would that affect the program?

    > it won't stop searching even if the path doesn't lead to a legal word,
	  meaning every index of the board matrix will be searched in all eight
	  directions, thus drastically increasing the number of comparisons
